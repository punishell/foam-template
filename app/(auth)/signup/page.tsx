...
export default function SignupPage(): React.JSX.Element {
	...
	useMemo(() => {
		if (systemSetting != null) {
			const signupByInviteONly = Boolean(
				systemSetting[SETTING_CONSTANTS.ALLOW_SIGN_ON_INVITE_ONLY] ===
					"true",
			);
	...
	return (
		<>
			...
			{!loading && !errorMsg && (
				<Container className="mt-[42px] pb-[42px] sm:mt-0 flex h-auto sm:h-full w-full sm:max-w-2xl flex-col items-center justify-center gap-6">
			...
			{!loading && errorMsg && (
				<Container className="mx-auto mt-28 flex w-full max-w-xl flex-col items-center justify-center gap-2 rounded-2xl border border-white border-opacity-20 bg-[rgba(0,124,91,0.20)] px-4 py-6 sm:px-[40px] sm:py-10 text-center text-white backdrop-blur-md">
