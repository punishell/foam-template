...
export default function Chat({ params }: Props) {
  ...
  const onDrop = React.useCallback(async (acceptedFiles: File[]) => {
    const files = acceptedFiles.map((f, i) => ({ file: f, preview: getPreviewByType(f).preview, type: getPreviewByType(f).type, id: String(i), name: f.name, size: formatBytes(f.size, 0) }));
    setImageFiles(files);
  }, []);
  ...
  const sendMessage = async () => {
    if (text != "" || imageFiles.length > 0) {
      await sendUserMessage(currentConversation?.recipient?._id, currentConversation?.sender?._id, MessageTypeEnums.TEXT, text, currentConversation.id, imageFiles);
      setText("");
      setImageFiles([]);
    }
  }
    ...
    <div className='flex flex-col h-full'>
      ...
      <div className="flex flex-row gap-2 w-full" {...getRootProps()}>
        <div className='flex items-end w-10'>
          <button className="border h-8 w-8 bg-[#008D6C1A] text-primary rounded-full flex items-center justify-center" onClick={open} >
            <Paperclip size={16} />
            <input {...getInputProps()} />
          </button>
        </div>
            ...
            <div className="w-full" key={i}>
              {!message.isSent ?
                <div className='flex flex-col mr-auto w-fit gap-2 max-w-[600px] px-5'>
                  ...
                  {message.content && <div className="mr-auto w-fit max-w-[600px] bg-[#ECFCE5] px-5 py-2 text-title rounded-r-[30px] rounded-tl-[30px] break-words whitespace-pre-line">
                    {message.content}
                  </div>}
                </div> :
                <div className='flex flex-col ml-auto w-fit gap-2 max-w-[600px] px-5'>
                  ...
                  {message.content && <div className="ml-auto w-fit max-w-[600px] text-white px-5 py-2 bg-[#007C5B] rounded-l-[30px] rounded-tr-[30px]  break-words whitespace-pre-line">
                    {message.content}
                  </div>}
                </div>
              }
    ...
    return (
      <div className='flex gap-2 bg-[#F7F9FA] w-fit h-fit p-4 rounded-lg'>
        <div className='flex flex-col w-10 items-center'>
          <Image className='min-w-[38px] min-h-[38px] rounded-lg' src={img?.file ? getPreviewByType(img.file).preview : getPreviewByTypeUrl(img?.url, img?.type).preview} alt="upload-picture" width={38} height={38} objectFit="contain" />
        </div>
        <div className='flex flex-col w-64 flex-1'>
          ...
          <p className='flex items-center text-sm text-body'>{img?.file ? `${img.progress || 0}%` : dayJs(img?.createdAt).format("DD mmm, YYYY")} <Dot size={20} /> <span>{img?.size}</span></p>
        </div>
        <div className='flex w-10 items-center'>
          {(img?.file || downloadAttachments.isLoading) ? <Loader size={20} className='text-primary animate-spin' /> : <DownloadIcon size={20} className='text-primary cursor-pointer' onClick={() => DownloadAttachment(img?.url)} />}
        </div>
      </div>
    )
  }
