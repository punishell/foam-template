...
const Message = ({
    ...
    return (
        <div
            className={` flex w-fit max-w-[37.5rem] items-end whitespace-pre-line break-words px-5 py-2 ${className}`}
        >
                ...
                >
                    {groupedMessages.map(([date, messagesForDate]) => (
                        <div
                            ...
                            key={date}
                        >
                            <h2 className="flex h-[48px] items-center text-center text-[#DEDEDE]">
                                {dayjs(date).format("MMMM D, YYYY")}
                            </h2>
                            {messagesForDate.map((message, i) => (
                                <div className="w-full" key={i}>
                                    {!message.isSent ? (
                                        <div className="my-1 mr-auto flex w-fit max-w-[37.5rem] flex-col px-5">
                                        ...
                                        <div className="my-1 ml-auto flex w-fit max-w-[37.5rem] flex-col !items-end px-5">
                                            {message.attachments.length > 0 && (
                                                <RenderAttachmentViewer
                                                    // @ts-expect-error ---
                                                    images={message.attachments}
