...
import { TalentJobModal } from '../jobs/job-modals/talent-job-modal';
import { ClientJobModal } from '../jobs/job-modals/client-job-modal';
...
const ActiveJobCard: React.FC<ActiveJobCardProps> = ({ id, talent, creator, title, description, progress, isCreator }) => {
  ...
  return (
    <div className="border-[#CDCFD0] bg-[#F9F9F9] gap-4 pl-2 px-4  flex border z-10 w-full rounded-2xl relative overflow-hidden">
      ...
      >
        {!isCreator ? <TalentJobModal jobId={id} closeModal={() => setIsUpdateModalOpen(false)} /> : <ClientJobModal jobId={id} closeModal={() => setIsUpdateModalOpen(false)} />}
      </SideModal>
    </div>
  );
};
...
export const ActiveJobs = () => {
  ...
  const { data: jobAssignedData, isFetched: assignedFetched, isFetching: assignedFetching, isError: assignedError } = useGetJobs({ category: 'assigned', status: 'ongoing' });
  const { data: jobCreatedData, isFetched, isFetching, isError } = useGetJobs({ category: 'created', status: 'ongoing' });
  const jobDataJoined = [...(jobAssignedData?.data || []), ...(jobCreatedData?.data || [])];
  const jobData = jobDataJoined.filter(f => f.inviteAccepted);

  const activeJobs = useMemo(() => (jobData || []).map((job, i) => {
    const deliverableTotal = job?.collections.filter(f => f.type == "deliverable").length;
    const deliverableTotalCompleted = job?.collections.filter(f => (f.type == "deliverable" && f.progress == 100)).length;
