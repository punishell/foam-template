  ...
  return (
    <SideModal isOpen={isOpen} onOpenChange={onChange} className="gap-6">
      ...
      {!is2FA ? <>
        ...
        <TwoFAInput isLoading={withdraw.isLoading} onComplete={finalSubmit} type={twoFa.type} close={() => _setIs2FA(false)} />
      }
    </SideModal>
  );
};
...
const TwoFAInput = ({ isLoading, type, onComplete, close }: { isLoading: boolean, type?: string, onComplete: (otp: string) => void, close: () => void }) => {
  const {
    handleSubmit,
    formState: { errors },
    control,
      ...
      <div className="flex items-center px-4">
        <div className="flex flex-col items-center w-full text-center p-6 border rounded-2xl">
          ...
          <form className="flex flex-col gap-2 w-full" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex">
              <Button type="submit" className="w-full justify-end self-end justify-self-end" disabled={isLoading} fullWidth>
