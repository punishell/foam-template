...
const Steps = ({ jobSteps, isEdit }: StepsProps): ReactElement => {
	return (
		<div className="flex h-fit flex-col gap-3 rounded-xl border border-line bg-white p-6">
			...
			<StepIndicator isComplete={jobSteps.description}>
				Description
			</StepIndicator>
			<StepIndicator isComplete={jobSteps.deliverables}>
				Deliverables
			</StepIndicator>
			<StepIndicator isComplete={jobSteps.classification}>
				Classifications
			</StepIndicator>
			{isEdit && (
				<StepIndicator isComplete={false}>
					Deposit Payment
				</StepIndicator>
			)}
		</div>
